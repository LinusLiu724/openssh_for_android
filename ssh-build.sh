...
helper()
{
    ...
    echo "  _PATH_SSH_PROGRAM   default is /system/xbin/ssh"
    exit 1
}

NCPU=$(grep -ci processor /proc/cpuinfo)
ANDROID_NDK_ROOT=${1:-"$HOME/data/zhi.liu/android-ndk-r8e"}
# ANDROID_NDK_ROOT=${1:-"$HOME/data/zhi.liu/android-ndk-r14b"}
_PATH_SSH_PROGRAM=${2:-"/system/xbin/ssh"}
